<?php
 ...
 checkAdminSession();
 // Lấy ID của brand cần sửa
$brand_id = isset($_GET['id']) ? (int)$_GET['id'] : 0;

// Lấy thông tin của brand
$stmt = $pdo->prepare("SELECT * FROM brands WHERE brand_id = ?");
$stmt->execute([$brand_id]);
$brand = $stmt->fetch(PDO::FETCH_ASSOC);

// Nếu không tìm thấy brand, chuyển hướng về trang danh sách
if (!$brand) {
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($name)) {
        ...
        if ($stmt->execute([$name, $brand_id])) {
            ...
            // Cập nhật lại thông tin brand sau khi sửa
            ...
            <div class="position-relative row form-group">
                ...
                <div class="col-md-9 col-xl-8">
                    <input required name="name" id="name" placeholder="Name" type="text"
                        class="form-control" value="<?php echo htmlspecialchars($brand['brand_name']); ?>">
                </div>
            </div>

            <div class="position-relative row form-group mb-1">
                                            <div class="col-md-9 col-xl-8 offset-md-2">
                                                <a href="brand.php" class="border-0 btn btn-outline-danger mr-1">
