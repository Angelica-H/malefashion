...
// Xử lý sắp xếp
$sort = isset($_GET['sort']) ? $_GET['sort'] : 'brand_id';
$order = isset($_GET['order']) ? $_GET['order'] : 'DESC';

// Xử lý phân trang
$page = isset($_GET['page']) ? (int)$_GET['page'] : 1;
$perPage = 5; // Số brand trên mỗi trang
$start = ($page - 1) * $perPage;

// Truy vấn lấy tổng số brand
...
// Truy vấn lấy danh sách brand
$query = "SELECT * FROM brands WHERE brand_name LIKE :search ORDER BY $sort $order LIMIT :start, :perPage";
...
" class="btn-shadow btn-hover-shine mr-3 btn btn-primary">
    ...
    <div class="card-header">
        <form method="GET">
            <div class="input-group">
                <input type="search" name="search" id="search"
                    placeholder="Search brands" class="form-control" value="<?php echo htmlspecialchars($search); ?>">
        ...
        <div class="btn-actions-pane-right">
            <div role="group" class="btn-group-sm btn-group">
                <a href="?sort=brand_name&order=ASC<?php echo $search ? "&search=$search" : ''; ?>" class="btn <?php echo $sort == 'brand_name' && $order == 'ASC' ? 'btn-primary' : 'btn-outline-primary'; ?>">Name A-Z</a>
                <a href="?sort=brand_name&order=DESC<?php echo $search ? "&search=$search" : ''; ?>" class="btn <?php echo $sort == 'brand_name' && $order == 'DESC' ? 'btn-primary' : 'btn-outline-primary'; ?>">Name Z-A</a>
                <a href="?sort=brand_id&order=DESC<?php echo $search ? "&search=$search" : ''; ?>" class="btn <?php echo $sort == 'brand_id' && $order == 'DESC' ? 'btn-primary' : 'btn-outline-primary'; ?>">Newest</a>
                <a href="?sort=brand_id&order=ASC<?php echo $search ? "&search=$search" : ''; ?>" class="btn <?php echo $sort == 'brand_id' && $order == 'ASC' ? 'btn-primary' : 'btn-outline-primary'; ?>">Oldest</a>
            </div>
        </div>
    </div>
    ...
    <div class="table-responsive">
        <table class="align-middle mb-0 table table-borderless table-striped table-hover">
            ...
            <tbody>
                ...
                <tr>
                    <td class="text-center text-muted">#<?php echo $brand['brand_id']; ?></td>
                    <td>
                        <div class="widget-content p-0">
                            <div class="widget-content-wrapper">
                                <div class="widget-content-left flex2">
                                    <div class="widget-heading"><?php echo htmlspecialchars($brand['brand_name']); ?></div>
                                </div>
                            </div>
                        </div>
                    </td>
                    <td class="text-center">
                        <a href="./brand-edit.php?id=<?php echo $brand['brand_id']; ?>" data-toggle="tooltip" title="Edit"
                        ...
                        <form class="d-inline" action="" method="post">
                            ...
                            <button class="btn btn-hover-shine btn-outline-danger border-0 btn-sm"
                                type="submit" name="delete_brand" data-toggle="tooltip" title="Delete"
            ...
            <ul class="pagination justify-content-center">
                <?php for ($i = 1; $i <= $totalPages; $i++): ?>
                    <li class="page-item <?php echo $page == $i ? 'active' : ''; ?>">
                        <a class="page-link" href="?page=<?php echo $i; ?>&sort=<?php echo $sort; ?>&order=<?php echo $order; ?><?php echo $search ? "&search=$search" : ''; ?>">
                            <?php echo $i; ?>
                        </a>
                    </li>
                <?php endfor; ?>
